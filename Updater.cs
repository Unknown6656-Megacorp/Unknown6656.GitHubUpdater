...
public sealed class GithubUpdater
{
    ...
    public GithubUpdater(string repo_author, string repo_name)
    {
        Client = new GitHubClient(new ProductHeaderValue($"{repo_author}.{repo_name}"));
    ...
    /// </returns>
    public async Task<bool> FetchReleaseInformationAsync()
    {
        try
        {
    ...
    /// <returns>Returns whether the the information could be fetched successfully <b>and</b> whether the update could be successfully performed.</returns>
    public async Task<bool> TryUpdateTo(Release release)
    {
        try
        {
